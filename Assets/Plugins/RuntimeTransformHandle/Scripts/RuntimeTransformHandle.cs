...
namespace RuntimeHandle
{
    ...
    public class RuntimeTransformHandle : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (Mouse.current.leftButton.isPressed/*Input.GetMouseButton(0)*/ && _draggingHandle != null)
            {
                _draggingHandle.Interact(_previousMousePosition, handleCamera);
                if (OnMove != null)
                    OnMove();
            }

            if (Mouse.current.leftButton.wasPressedThisFrame/*Input.GetMouseButtonDown(0)*/ && handle != null)
            {
                _draggingHandle = handle;
                _draggingHandle.StartInteraction(hitPoint, handleCamera);
                if (OnMove != null)
                    OnMove();
            }

            if (Mouse.current.leftButton.wasReleasedThisFrame/*Input.GetMouseButtonUp(0)*/ && _draggingHandle != null)
            {
                _draggingHandle.EndInteraction(handleCamera);
                _draggingHandle = null;
                if (OnMove != null)
                    OnMove();
            }

            _previousMousePosition = Mouse.current.position.ReadValue();// Input.mousePosition;
            _previousMousePosition.z = handleCamera.nearClipPlane;
            if (target == null)
                return;
        ...
        void LayerSet(int Layer, GameObject Obj)
        {
            Obj.layer = Layer;
            for (int i = 0; i < Obj.transform.childCount; i++)
            {
                LayerSet(Layer, Obj.transform.GetChild(i).gameObject);
            }
        }
        ...
        private void GetHandle(ref HandleBase p_handle, ref Vector3 p_hitPoint)
        {
            ...
            mousePos.z = handleCamera.nearClipPlane;
            Ray ray = handleCamera.ScreenPointToRay(mousePos);
